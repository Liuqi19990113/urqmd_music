...
namespace Transform{
  ...
  /*LiuQi's turn*/
  void WriteFlowMUSIC_IP2d(const EPTensor&tensor,const std::string&output_path){
    auto flow=tensor.GetFlow();
    std::string output_file;
    output_file=output_path+"MUSIC_IP2dinit.txt";
    std::ofstream output(output_file.c_str());
    /*First,let's write head line*/
    output<<"# dummy 1 netamax= 1 xmax= "<<Ex_M_bin[1]<<" ymax= "<<Ex_M_bin[2]<<" deta= 0 "<<"dx= "<<Ex_Dx[1]<<" dy= "<<Ex_Dx[2]<<std::endl;
    /*Then,let's write slices*/
    for(unsigned i=0;i<Ex_M_bin[1];i++){
      for(unsigned j=0;j<Ex_M_bin[2];j++){
        unsigned k=search_bin(0,Ex_M_down[3],Ex_M_up[3],Ex_M_bin[3]);
        double Temp_ed=flow[0][i][j][k];
        double Temp_ux=flow[1][i][j][k];
        double Temp_uy=flow[2][i][j][k];
        double Temp_utau=sqrt(1. + Temp_ux*Temp_ux + Temp_uy*Temp_uy);
        output<<"0 "<<Ex_M_down[1]+i*Ex_Dx[1]<<' '<<Ex_M_down[2]+j*Ex_Dx[2]<<' '
        <<Temp_ed<<' '<<Temp_utau<<' '<<Temp_ux<<' '<<Temp_uy<<' '
        <<"0 0 0 0"<<std::endl;
      }
    }
    output.close();
